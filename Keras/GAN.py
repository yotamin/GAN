...
class GAN:
    ...
    def __init__(self, discriminator_layers_dims,
                 generator_layers_dims,
                 x_input_shape=(28, 28, 1),
                 g_latent_dim=100,
                 discriminator_optimizer=Adam(0.0001),
                 generator_optimizer=Adam(0.0001)):

        self.x_input_shape = x_input_shape
        self.discriminator_layers_dims = discriminator_layers_dims
        self.generator_layers_dims = generator_layers_dims
    ...
    def build_discriminator(self):
        ...
        x = Flatten()(input)
        for layer_dim in self.discriminator_layers_dims:
    ...
    def build_generator(self):
        ...
        x = input
        for layer_dim in self.generator_layers_dims[:-1]:
            # Regularization can be added
            x = Dense(units=layer_dim, activation='relu')(x)
        x = Dense(units=self.generator_layers_dims[-1], activation='tanh')(x)
        output = Reshape(self.x_input_shape)(x)
        return Model(inputs=input, outputs=output)
...
if __name__ == '__main__':
    ...
    gan = GAN(discriminator_layers_dims=[600, 300], generator_layers_dims=[300, 600, 784])
